...
# for (size_t i=0; i < state.size(); i++) if (std::
#     isnan(state[i])) bTrouble = true;
# if (bTrouble)
#     {
#         reward = -1000;
#     printf("Caught a nan!\n");
...
class FISHTestEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # self.obs[13] = self.train_fish.GetFishPositionX(0)
        # self.obs[14] = self.train_fish.GetFishPositionY(0)
    ...
    def step(self, action):
        ...
        if done == False:
            if self.action_time_steps > 99:
                ...
                reward = reward + 10.0
        # elif self.train_fish.GetFishPositionX(0) < 0 or self.train_fish.GetFishPositionY(0) > 0.8: # mark all points need to be in the fluid threshold
        #     done = True
        #     reward = reward - 10.0
        ...
        file.write('  fish_x  ')
        file.write(str(self.fish_dx + self.goal_x)) # self.train_fish.GetFishPositionX(0)
        file.write('  fish_y:  ')
        file.write(str(self.fish_dy + self.goal_y)) # self.train_fish.GetFishPositionX(0)
        file.write('\n')
        file.close()
